...
def chapter4(df: pd.DataFrame) -> tuple[str, str]:
    ...
    for i in range(n_iter):
        idx = rng.choice(len(s), size=sample_size, replace=True)
        means[i] = s[idx].mean()
    ...
    ax.axvline(pop_mean, color='#D55E00', linewidth=1.2,
               label='経験分布の平均')
    ...
    text = dedent(f"""
        - 復元抽出によるリサンプリング: {n_iter} 回, 標本サイズ {sample_size}
        - 経験分布の平均: {fmt_num(pop_mean)}
...
def chapter8(df: pd.DataFrame) -> str:
    """主要な変数ペアの相関係数."""
    pairs = [
        ('budget',          'views_30d', '製作費 vs 視聴回数',           '弱〜中 (0.3〜0.5)'),
        ('rec_impressions', 'views_30d', 'おすすめ表示回数 vs 視聴回数', '中〜強 (0.65〜0.85)'),
    ...
    lines.append('')
    lines.append('注: おすすめ表示回数の設計目安は, 実データ確定値 r=0.8033 に合わせて 0.65〜0.85 へ調整した.')
    return '\n'.join(lines)
...
def chapter9(df: pd.DataFrame) -> tuple[str, str]:
    """製作費 → 視聴回数 の単回帰."""
    ...
    path = FIG_DIR / 'ch09_regression.png'
    fig.savefig(path)
    plt.close(fig)
    ...
    text = dedent(f"""
        ...
        傾きは正で, 決定係数は約0.13. 製作費だけでは説明しきれない散らばりが
        散布図から読み取れる.
    """).strip()
...
def main() -> None:
    ...
    md.append(ch6)
    md.append('')
    md.append('### 8章: 主要な変数ペアの相関係数')
    md.append('')
    md.append(ch8)
    md.append('')
    md.append('### 9章: 製作費 → 視聴回数 単回帰')
    md.append('')
    md.append(ch9_text)
    md.append('')
    md.append(f'![単回帰散布図]({ch9_fig})')
    md.append('')
